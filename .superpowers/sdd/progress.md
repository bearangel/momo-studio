...
export-rich P0 修复: 长流压缩后导出只剩摘要快照。根因=alignVisibleEntries 沿用 2026-09-06「分段替换父」旧语义, 显示侧 2026-09-08 已改为「分段行剔除/父行唯一主体」(MessageList.tsx:134 注释), 导出侧脱节。修复=对齐显示语义(segmentOf 行一律剔除, 删替换+孤儿兜底, 子展开同法)。回归锁=真 DB 集成复现用例(父行全文+工具块在/摘要不在/头计数2) + 2 旧语义单测改写。门禁: 红3→绿102(im)/1621(全量串行)/typecheck双clean。commit 见 git log。
turn-mandate T1: complete (commit 0cc7a9e, review clean——Spec ✅ + Approved; 12/12 → 1625/1625 全量 + typecheck 双clean; Minor ×3: ①TodoTools 类docstring 摘要格式示例未含[u]/[a] ②pending-user 计数分支未直接用例[brief 预定测试所限] ③renderer 镜像 TodoItem(types.d.ts:591) 未加 source 字段——留终审triage)
turn-mandate T2: complete (commits 18c871e + ad61560 审查修复, review clean after fix loop——Spec ✅ + Approved; 5/5 含 7 条反断言 + tests/agent/ 695/695 + typecheck 双clean; 审查循环 1 轮: Important×2[注释字面串×3/compact param description 自相矛盾]均已修复并被参数级断言锁死; Minor 遗留×2: ①消费端接线无回归锁[→T4 harness 补] ②计划五处残留枚举 vs 实况四处[:610/622/659/675]——T4 brief 按实况枚举)
turn-mandate T3: complete (commit aab9ad4, review clean——Spec ✅ + Approved; mandate-hint 2/2 + tests/agent/ 697 + 全量 1632/1005 + typecheck 双clean; TDZ 前移独立评判零行为差异; 聚焦检查证实 messages[0] 重写与 dispatch 段无冲突[操作 ctx.tools 非 messages]; Minor ×3: ①brief 内部自相矛盾[实现「不要」vs 测试「勿」]实现者以测试为准但未上报 ②报告 §3.3 称 drain 后第二次 refresh 冗余——实为承载性[纳入新 steers], T4 接线时勿被误导 ③测试文件缺 EOF 换行)
turn-mandate T4: complete (commits 2a1a6c7 + f52eab7 审查补锁, review clean after fix loop——Spec ✅ + Approved; compact-wrapup 6/6[三态文案K值/dispatch子路径8参第三态/过短拒绝双锁] + tests/agent 701 + 全量 1636 + typecheck 双clean + 红线 grep 0 命中; chunk 捕获接线经证伪式验证[模块级 sendStreamChunk→process.send]; 审查循环 1 轮: Important×2[contract6 K文案/消息删除无锁, contract1 dispatch 子路径 0 覆盖]均补锁; Minor 遗留×2: ①contract7 task_complete 运行时文案无锁[→T8] ②pendingUserItems/hasPendingUserTodos 谓词一致性无机械锁[→T5])
turn-mandate T5: complete (commit 6b9bada, review clean——Spec ✅ + Approved; scope-gate 6/6 + delegation 6/6 + tools 167/167 + 全量 1644 + typecheck 双clean; delegation 测试改动裁定为意图精确保存[K1 断言原样+覆盖矩阵闭合: 无指派/有指派无挂靠/有指派有挂靠]; audited() 聚焦检查: warning 完整透传+审计截断线内; 谓词一致性锁落地[T4 遗留②闭环]; Minor ×3: ①两新文件缺 EOF 换行 ②p2p mock 省略[隐式依赖 broadcast no-op] ③谓词 describe 无 afterEach[无实际污染面])
turn-mandate T6: complete (commits 58f8d3e + 2746072 审查修复, review clean after fix loop——Spec ✅ + Approved; session-command 7/7 + tests/im 109/109 + tests/memory 193/193 + typecheck 双clean; extraction 重构零漂移/ack 落库形状对齐/无静态环/executionSessionId 语义成立均实证; 审查循环 1 轮: Important×1[COMPACT_WINDOW 取数方向陷阱,>1000 消息会话摘要陈旧化——brief verbatim 遗留]已修[listRecentMessagesBySession+双回归锁]; Minor 遗留×2: prompt 限定语精简[brief 指定] 车道排队窗口窄[benign]; T7 契约基准= report §5)
turn-mandate T7: complete (commit 3d907f2, review clean——Spec ✅ + Approved; 63/63 + 22/22 + renderer 1008/1008 + typecheck 双clean; IPC 契约三面[handler/preload/types.d.ts]逐字对齐交叉验证通过; 四拦截语义+分支顺序+插入点均核; Minor ×3: ①types.d.ts SessionSummary 块 12 行意外缩进漂移[纯空白,下次触碰还原] ②hint 渲染无断言[brief 范围外,3 行用例可闭环] ③/compact 带参数判未知命令[spec 未定义,备注])
turn-mandate T8: complete (无 commit; 控制器亲验四门禁 PASS——typecheck 双 clean / 红线 grep「继续工作」运行时路径零命中[排除测试反断言] / electron 200 文件 1651 用例全绿 / renderer 107 文件 1008 用例全绿; macOS 真机四场景验收剧本留主机)
终审（whole-branch, oracle）: With fixes——跨任务接缝零漂移[source 谓词四消费点/IPC 三面契约]、spec §5 代码级全覆盖、回归面独立跑绿[steer×2+dispatch-parallel+segment-restart+44/85 分支测试]。审查循环: 终审 Important×1[renderer TodoItem 缺 source, 铁律4 两端成对]已修(a5b28be, 5 文件零行为变更, 控制器亲验 hunk)。Minor 12+6 项 triage: FIX 前置全清[#1/#10 同 commit], 其余 DEFER-OK 有理由。macOS 真机四场景剧本留主机(spec §11-1/5 唯一未闭环项)。
turn-mandate SDD 执行完毕: 8 任务×(实现+独立审查)双循环 + 4 个审查修复轮[T2×1/T4×1/T6×1/终审×1] + T8 控制器亲验。12 commits(e9ef4de..a5b28be)。门禁终态: typecheck 双 clean + electron 200/1651 + renderer 107/1008 + 红线 grep 运行时零命中。
─── compaction-overhaul（spec 2026-09-09，分支 feat/compaction-overhaul）───
compaction T1: complete (commits 453d227 + 36b8989, review clean——Spec ✅ + Approved; 目录 23 条目/六契约四点对齐/buildSpawnOpts async 化 5 调用点全 await/迁移 SQL 逐字; 审查条件性 Important[upsert 或清窗口]经控制器 grep 定论为 INSERT OR IGNORE 不成立; Minor ×5: 报告条目计数偏差/spec 内联注释未带/-latest 别名 fail-safe null/输入错误路径不重置/死种子; 全量 1680+1012 绿+typecheck 双clean+renderer lint 过[electron 1 预存未混修])
compaction T2: complete (commit ded3816, review clean——Spec ✅ + Approved; 37/37 新用例 + 1718/1718 + 1012/1012 + typecheck 双clean; 系数由纯 CJK 用例锁死[÷4 回归会得 3≠7]; 纯度核查过[仅 import type]; Minor ×4: ①混合用例区间放过 ÷4 回归[套件整体锁死,brief 字面要求区间] ②交接注记①措辞与代码不符[serialize 容忍输出 [系统]: 行,T3 派发已带修正] ③CJK 增补平面 U+20000+ 未覆盖[罕见] ④「(无)」占位无测试锁[T3 可顺手补])
compaction T3: complete (commit d631e55, review clean——Spec ✅ + Approved; 1743 全绿 + typecheck 双clean; IPC 桥四点同 commit 且 respond 注入可测; 三裁定判正当[①prior 自读=签名收紧无行为差异 ②SUMMARY_MAX_LEN 补 export 单源 ③文案不变保守解读]; 聚焦检查: respond 异常路径全回 ok:false/pending 双路径 delete 无泄漏/covered_until 严格大于语义精确无漏拉重拉; Minor ×4: ①process.send 同步 throw 边缘[10s 自愈] ②ack 计数含排除尾部 ③conversation 容忍收敛 ④/compact 末轮边界为 brief 裁定[spec keep 预算版留 T5])
compaction T4: complete (commit 5f1f04d, review clean——Spec ✅ + Approved; 11 新用例真 DB + tests/memory 95 + 全量 1754/1012 + typecheck 双clean; 三裁定独立查证成立[prune 休眠=spec 级缺口非实现错配/环真实预案获批 SQL 同形/注入形状 spec 明文]; prune-before-injection 顺序正确且组合用例锁定; steer 测试隔离修复判被迫正当; Important ×2 登记: ①§8 prune 生产休眠[tool_call_result 无 messages 行生产者]→T7 验收 caveat ②T5 交接: [此前对话压缩摘要] 前缀跳过规则+尾扫防误判; Minor ×3: SQL 双份待收敛/beforeTs 分页×注入无实害/ASC 最早 N 窗口 pre-existing)
compaction T5: complete (commits 99a6cb6 + a4a033a 审查修复, review clean after fix loop——Spec ✅ + Approved; compact-auto 7 + wrapup 6 + copy-neutral 4 + 全量 1760/1012 + typecheck 双clean; 审查循环 1 轮: Critical×1[尾部选择切断 assistant(toolCalls)/tool 对→孤儿 tool→provider 400,消费侧四映射点实证无净化]已修[边界式判定:切点不落 tool,不变式封闭,严格强于审查建议]+扫描式回归锁; 四裁定成立[fresh 子 agent no-op 测试实证/无操作化 warn/子 agent 失压缩 T6 兜底/IPC 迁出 mock 净化]; Important-1 遗留[coveredUntil=Date.now() 跨回合过覆盖未摘要尾部]→折入 T6; Minor 遗留×3: warn 文案误导/chip IPC 延迟+abort 不感知/锚点跳过专项锁已补)
compaction T6: complete (commit f9d98ae, review clean——Spec ✅ + Approved; overflow-recovery 7/7 + tests/agent 749 + 全量 1767 + typecheck 双clean + 红线 0; coveredUntil 精确化[T5 遗留 Important-1 闭环: WeakMap 引用跟随 convCtx timestamp/turnStart-1 双源,c1 红=旧 bug 精确复现]; 三裁定判正当[重放无条件=spec 语义且 steer 场景必需/RE 启发式有界兜底/主机冒烟待验]; 聚焦检查: 重放重复语义 adjudicated/flag 回合级无泄漏/auto 压缩不误触/WeakMap slice 稳定)
compaction T7: complete (无 commit; 控制器亲验三门禁 PASS——typecheck 双 clean / 红线 grep 零命中 / electron 211 文件 1767 + renderer 107 文件 1012 全绿; macOS 真机剧本[长任务自动压缩/跨回合摘要注入/窗口未知不自动压/溢出恢复]留主机; §8 prune 生产休眠 caveat 已登记)
终审（whole-branch, oracle 级）: Ready to merge=Yes——六接缝独立源码验证无错配, 契约测试/错误路径达机械标准, 控制器复核亲跑全绿。审查循环: 收尾 bundle 252ddfd（spec §8 prune caveat + §4.4 wire 勘正 + COMPACTION_SUMMARY_MAX_LEN=4000 解耦[终审 Important-2 提前落地, tests/compaction 49/49 + typecheck 双clean 控制器亲验]）。Important 遗留×2[spec 级设计债, 合并后跟进]: ASC 最早 N 窗口语义[pre-existing] / §8 prune 生产启用需消息持久化改造。Minor 19 项 triage 全 DEFER-OK。
compaction SDD 执行完毕: 7 任务×(实现+独立审查)双循环 + 3 审查修复轮[T5 Critical 孤儿工具对/T5 补锁/终审收尾] + T7 控制器亲验。10 commits(e1e602d..252ddfd)。门禁终态: typecheck 双 clean + electron 211 文件 1767 + renderer 107 文件 1012 + 红线 grep 运行时零命中。
