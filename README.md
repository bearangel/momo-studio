...
> **版本号说明**：下方各 v2.x 条目是**研发账本**（特性分组叙事），非发布史。产品版本号自
> 2026-09-13 起与账本解耦——研发期停在 prerelease 号（`2.1.0-alpha.N`），发正式版时才定
> 终号（策略见 `docs/dev/release.md`「研发期版本号策略」）。上一正式版：**v2.0.0**。
...
- **信任卡 + 设置分类** — trust 三态 ask/always/deny：ask 且 agent 首调浏览器工具时**工具调用阻塞等待**右下角信任卡（本次会话 / 永久允许 / 取消——用户决定直接驱动 agent 走向：允许则挂起中的调用继续执行，取消则以「用户已拒绝」失败；同 ws 并发工具只推一张卡）；3 分钟未应答降级为拒绝 + 重试指引（超时后补点卡 = 为下一次调用授权）；设置页「浏览器」分类——信任模式 / 域名黑白名单（子域匹配，白名单优先）/ evaluate 开关 / 清除浏览数据（清 partition storage）；workspace_settings 表 migration 034
