...
- **isInsideDir 是唯一目录边界判定入口，禁手写 startsWith 比对**——适用域：**workspace / browser / journal / git 边界模块**（v2.10 已收敛六模块七处：5 处 startsWith + 2 处 `split/join`）。存量豁免（记 **v2.10.x 待办收敛，各配 win32 测试**）：skill 域两处（`skill/zip-uploader.ts` 条目解包防御 / `skill/loader.ts` 缓存 anchor）与 realpath anchor 链（`files/workspace-fs.ts` 最近存在祖先 anchor 比较；browser `policy.ts` 的 anchor 已转换不算存量）——新增代码一律禁手写。「child 是否在 root 内」一律经 `platform/paths.ts` 的 `isInsideDir`（两侧 resolve 归一 + win32 盘符与目录段大小写不敏感 + UNC 等价 + `path.sep` 边界防 `..foo.txt`/兄弟前缀误伤），POSIX 相对化一律 `toPosixRelPath`（禁手写 `split(path.sep).join('/')`）。手写比对在 win32 下必然漏大小写 / UNC / 混合斜杠语义。需要显式平台分叉的调用方传第三参 `opts.win32`（或用 `PATH_SEMANTICS_WIN32` 常量按当前 path 模块语义分叉），不要读 `process.platform` 自行判定
...
- **electron-builder 选项落段纪律（win vs nsis）**——`perMachine` / `oneClick` / `allowToChangeInstallationDirectory` 是 **nsis 段**选项（installer 行为），`icon` / `target` / `artifactName` / `signtoolOptions` 是 **win 段**选项（打包产物与签名）——放错段会被 app-builder-lib **静默忽略**（构建不报错、选项不生效）。**签名配置一律嵌套在 `win.signtoolOptions` 内**（`certificateFile` / `certificatePassword`，或经 env `CSC_LINK` / `WIN_CSC_KEY_PASSWORD` 注入）——顶层 `win.certificateFile` 写法**在 v26 已不生效**：实现（app-builder-lib 26.15.3 `windowsSignToolManager.js:67`）只读 `signtoolOptions.certificateFile`，schema `WindowsConfiguration` 为 additionalProperties:false，d.ts 里的顶层声明是 legacy 残留（终审 I1 实测纠偏：此前「certificateFile 定义于 WindowsConfiguration」的 Task 4/5 结论正是被该残留误导）。改 build 配置前先确认选项归属段，拿不准查 schema 与实现读取路径，不要「看着像就放」
