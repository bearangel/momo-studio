...
```bash
timeout 90 xvfb-run -a --server-args="-screen 0 1280x800x24" npx pnpm@9.0.0 dev
```

Expected: 输出包含 vite ready 与 electron 启动日志，进程存活至 timeout（exit 124）；无 CSP 违规报告、无 theme-boot.js 加载失败。人工补充项（macOS 主机）：设置 → 外观切换三态，全 app 即时换肤且重启记忆。
