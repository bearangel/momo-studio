...
grep -c 'max-w-\[70' /tmp/opencode/tw-probe.css && grep -c 'w-\[117px\]' /tmp/opencode/tw-probe.css
```

Expected: 两个 grep 均输出 `1`——Tailwind 3.4 对**静态出现**的任意值 class 正常生成 CSS。结论：历史债「任意值 class 不生成 CSS」的根因是**动态字符串拼接 class**（JIT 扫描器看不见运行时才拼出的字符串），规范文档（Task 20）据此写禁令。若 grep 输出 `0`：升级为配置问题，停下来在终端报告排查结果，不得继续后续任务。
