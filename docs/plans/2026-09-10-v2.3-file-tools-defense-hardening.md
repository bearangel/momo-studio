...
Run: `cd electron && npx pnpm@9.0.0 vitest run tests/agent/tools/apply-patch-tools.test.ts`
Expected: 10 passed（含 add-then-fail 严格回滚新增文件用例，由 fix subagent ec9b3a8 补充）
...
### Task 6：ToolContext 加 readTracker 可选字段（必须先于 Task 5）
...
### Task 5：edit_file / write_file 接入 ReadTracker（依赖 Task 6 完成）

**Files:
- Modify: `electron/src/main/agent/tools/file-tools.ts:129-255`（改造 executeFileTool 签名 + edit_file / write_file 分支）
- Test: `electron/tests/agent/tools/file-tools-read-gate.test.ts`（新增）

**Interfaces:**
- Consumes: `ReadTracker.assertRead(sid, parentSid, path)` from Task 1, `ctx.readTracker` from Task 6

**前置依赖**：Task 6 必须已 commit（ToolContext 已含 `readTracker` 字段）。

**关键改造点**：现有 `executeFileTool(toolName, args, wsFs)` 签名改为 `executeFileTool(toolName, args, ctx)`，因为需要 `ctx.readTracker` 与 `ctx.streamSessionId` / `ctx.parentStreamSessionId`。`FileTools.execute` 调用时直接传 `ctx`（不再 `ctx.wsFs`）。

- [ ] **Step 1：读 file-tools.ts 现有结构**

Run: `read /workspace/electron/src/main/agent/tools/file-tools.ts`（确认现有 executeFileTool 签名与各分支结构）
...
**关键修改 1**：`executeFileTool` 签名从 `(toolName, args, wsFs)` 改为 `(toolName, args, ctx)`，内部用 `const wsFs = ctx.wsFs`。

**关键修改 2**：write_file / edit_file 分支加 ReadTracker 守门。

**关键修改 3**：edit_file 失败信息替换为 formatEditError（含原文 5KB + 行号 + read_file 建议）。

```typescript
// electron/src/main/agent/tools/file-tools.ts（增量修改）
// v2.3 改造点：executeFileTool 接 ctx；edit_file / write_file 接入 ReadTracker 守门；
// edit_file 失败信息替换为 formatEditError。

import { ReadTracker } from './shared/read-tracker';
import { formatEditError } from './shared/edit-recovery';

// 关键修改 1：executeFileTool 签名改为接 ctx
export async function executeFileTool(
  ...
  const wsFs = ctx.wsFs;
  switch (toolName) {
    // read_file / list_files / mkdir / rm / mv / exists 等 case 不变（仅去掉末尾 wsFs 引用，改为 wsFs.xxx）
    ...
    case 'edit_file': {
      ...
      if (firstIdx === -1) {
        // v2.3 失败信息增强：formatEditError 含原文 5KB + 行号
    ...
    // ... 其他 case 不变（mkdir / rm / mv / exists），仅把 wsFs.xxx 前的 wsFs 改为局部 const ...
  }
}

// FileTools.execute 直接传 ctx（不取 ctx.wsFs）
...
**集成点**：
- 现有 `FileTools.execute` 改为传 `ctx`（不再取 `ctx.wsFs`）
- `executeFileTool` 内部首行 `const wsFs = ctx.wsFs` 保持既有 `wsFs.xxx` 调用风格
...
git commit -m "feat(tools): edit_file / write_file 接入 Read-before-Edit 强阻塞守门 + 失败信息增强"
```
