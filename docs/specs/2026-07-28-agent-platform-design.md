...
> **⚠️ 本文档为 v1.x 架构设计，自 v2.0.0 起已被取代。**
>
> 现行架构见 `docs/specs/2026-08-23-v2.0.0-platform-refactor-design.md`（v2.0.0 起所有 2.x 实现的依据）。
> 关键差异：v2.0.0 已彻底移除 Matrix/Tuwunel 全家（matrix-js-sdk、Conduit 子进程、bot 注册器、Space/room 关联列），传输层内迁为进程内事件分发；本文中的 IM 协议、bot 账号、P2P 协调服务等设计已被新方案覆盖。**正文保留仅作历史参考，不反映 v2.0.0 代码现状。**

| 字段 | 值 |
|---|---|
| 文档版本 | v1.0 |
| 创建日期 | 2026-07-28 |
| 状态 | **Superseded by v2.0.0 spec**（正文仅作历史参考） |
