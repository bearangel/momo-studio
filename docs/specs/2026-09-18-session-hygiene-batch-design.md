...
**决策**：新用户消息（新 streamSessionId）不从上一轮携带未完成待办。理由：mandate 按 轮授权（spec §5.1），自动携带会把上一轮的旧授权注入新轮 mandate——正是 mandate 死锁（F1，同一请求双倍执行）的诱因形态。跨轮历史清单可经消息流内联清单与头部任务按钮浮层回看；未来如需「继续上次任务」语义，应走显式指令（如 /continue 命令重播种），不走隐式携带。

### 2c. 会话 todo 聚合视图（renderer）

> **结果（2026-09-18 当日主机验收）**：未采纳，实现已回退——会话顶部常驻总览挤占主区空间，用户裁定不需要；待办可见性由气泡内联清单 + 头部任务按钮（TaskProgressButton）浮层承担。以下为落稿记录，留档备查。
