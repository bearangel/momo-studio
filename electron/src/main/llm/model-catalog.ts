...
// 条目顺序敏感——专项条目（如 1M 变体、glm-4.6）必须排在通配条目之前。
...
const CATALOG: CatalogEntry[] = [
  ...
  // gpt-5 全系：400K 总窗口（272K 输入 + 128K 输出）。
  {
    platform: 'openai',
  ...
  // 智谱 GLM（bigmodel.cn 模型文档，2026-09 查证）：glm-4.6 起 200K 上下文，
  // 96K 最大输出；glm-4.5 及更早 4.x 为 128K。4.7 等未收录新版本落到通配条目。
  {
    platform: 'openai',
    pattern: /^glm-4\.6/,
  ...
  // DeepSeek（api-docs.deepseek.com，2026-09 查证）：128K 上下文；
  // chat 8K 输出，reasoner（R1-0528 起）32K 输出。
  {
    platform: 'openai',
  ...
  // 通义千问（阿里云百炼模型列表，2026-09 查证）：qwen-plus 131,072 上下文；
  // qwen-max 历史档 32K（qwen3-max 新档请用户列覆盖）。输出均 8K。
  {
    platform: 'openai',
    pattern: /^qwen-plus/,
  ...
  // Kimi K2（platform.moonshot.cn 模型文档，2026-09 查证）：128K 上下文，8K 输出。
  {
    platform: 'openai',
    pattern: /^kimi-k2/,
    limits: { contextWindow: 128000, outputTokens: 8192 },
  ...
  // 2.5 系 1M 上下文 / 65,536 输出；2.0-flash 1M 上下文 / 8,192 输出。
  {
    platform: 'openai',
