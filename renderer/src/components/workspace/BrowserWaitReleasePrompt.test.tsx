// renderer/src/components/workspace/BrowserWaitReleasePrompt.test.tsx
//
// 接管释放提示卡测试（spec 2026-09-14 §4.3；2026-09-15 M2 改 CenterPromptLayer 居中卡）：
...
import { BrowserWaitReleasePrompt } from './BrowserWaitReleasePrompt';
...
describe('BrowserWaitReleasePrompt（Tier A 释放等待卡）', () => {
  it('非 agent-waiting-release kind（trust-request）→ 不挂载', () => {
    const { push } = armOnBrowserNotice();
    const { container } = render(<BrowserWaitReleasePrompt />);
  ...
  it('kind=agent-waiting-release → 渲染卡片（标题 + 推送 text + 按钮「释放并继续」）', () => {
    const { push } = armOnBrowserNotice();
    render(<BrowserWaitReleasePrompt />);
    push({ kind: 'agent-waiting-release', text: 'agent 等待浏览器释放（最长 30 秒）', workspaceId: 'w1' });
    expect(screen.getByTestId('browser-wait-release-prompt')).toBeInTheDocument();
  ...
  it('onBrowserState 推送 takeover=agent（目标 ws 匹配）→ 卸载', () => {
    ...
    expect(screen.queryByTestId('browser-wait-release-prompt')).toBeNull();
  });
  ...
  it('点击「释放并继续」→ releaseTakeover(notice.workspaceId) + 卡片消散', async () => {
    ...
    await waitFor(() => expect(screen.queryByTestId('browser-wait-release-prompt')).toBeNull());
  });
